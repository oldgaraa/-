...
const double gc = 9.80665;                   // Нормальное ускорение свободного падения
...
double betta[Natm] = { -6.5E-3,      -6.5E-3,   0.0E-3,        1.0E-3,        2.8E-3,       0.0E-3,       -2.8E-3,      -2.0E-3,      0.0E-3 };
double pp[Natm] = { 0 };
...
double fT(double h)
{
	int i = findi(h);
	double H = fH(h);
	return (Tp[i] + betta[i]*(H - Hp[i]));
}
double fp(double h)
{
	int i = findi(h);
	double H = fH(h);
	if (betta[i] == 0) return (pp[i]*exp(-gc*(H - Hp[i]) / (R*fT(h))));
	else return(pp[i]*pow(1 + betta[i]*(H - Hp[i]) / Tp[i], -gc / (R*betta[i])));
}
double fro(double h)
{
	return (fp(h) / (R*fT(h)));
...
void Atm_GOST()
{
	pp[0] = pc / (pow(1 + betta[0] * (0 - Hp[0]) / Tp[0], -gc / (R*betta[0])));
	for (int i = 1; i <= Natm - 1; i++)
	{
		if (betta[i-1] == 0) pp[i]= (pp[i-1] * exp(-gc*(Hp[i] - Hp[i-1]) / (R*Tp[i])));
		else pp[i]=(pp[i-1] * pow(1 + betta[i-1] * (Hp[i] - Hp[i-1]) / Tp[i-1], -gc / (R*betta[i-1])));
	}
